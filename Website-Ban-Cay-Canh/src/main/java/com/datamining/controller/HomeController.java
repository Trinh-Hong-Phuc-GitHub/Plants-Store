...
//		List<Product> list = pService.findAll();
		...
		if(req.getRemoteUser() != null) {
			...
			if (us != null) {
				...
				// Xử lý trường hợp us là null, có thể hiển thị thông báo lỗi hoặc thực hiện hành động phù hợp khác.
				model.addAttribute("error", "User (us) is null");
			}
		}
	...
	public String account_info(Model model,HttpServletRequest req)
	{
		if(req.getRemoteUser() == null)
		{
			...
			// Kiểm tra xem us có khác null hay không
			if (us != null) {
				int usId = us.getId();
				model.addAttribute("user_id", usId);
				...
				// Xử lý trường hợp us là null, có thể hiển thị thông báo lỗi hoặc thực hiện hành động phù hợp khác.
				model.addAttribute("error", "User (us) is null");
				return "user/security/my-account"; // Thay thế bằng trang lỗi của bạn
			}
		}
	}
